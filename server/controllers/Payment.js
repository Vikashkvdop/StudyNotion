...
exports.capturePayment = async (req, res) => {
  ...
  for (const course_id of courses) {
    try {
      ...
      if (typeof course.price !== "number") {
        return res.status(500).json({ success: false, message: "Invalid course price." });
      }

      total_amount += course.price;
    } catch (error) {
      console.error("Error while processing course:", error);
      return res.status(500).json({ success: false, message: error.message });
    }
  }

  // Validate total_amount before proceeding
  ...
  const options = {
    amount: Math.round(total_amount * 100), // Razorpay expects paise
    currency: "INR",
    receipt: `rcpt_${Date.now()}`, // Valid receipt format
  };
  ...
  try {
    ...
    res.json({
      success: true,
      id: paymentResponse.id,           // Razorpay order_id
